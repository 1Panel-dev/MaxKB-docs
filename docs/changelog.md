...
!!! Abstract "新增功能 :star2:"
    ...
    - 应用：新增支持文档标签检索、变量拆分、变量聚合、参数提取、视频理解节点；
...
!!! Abstract "功能优化 :sunflower:"
    ...
    - 应用：上传文件支持视频文件类型；
    - 应用：图片理解、图生视频、视频理解节点的选择文件参数支持 URL 地址；
    - 应用：变量赋值节点可作为结束节点；
    - 应用：工作流编排页面支持批量选中节点；
    - 应用：接口参数增加描述字段；
    - 应用：表单收集中多选框组件支持自定义输入；
    - 应用：根目录显示当前工作空间下所有应用，支持全局搜索；
    - 应用（X-Pack）：应用接入企业微信、飞书、钉钉后，支持用户免密登录；
    - 工具：根目录显示当前工作空间下所有工具，支持全局搜索；
    - 工具：自定义类型参数支持变量解析；
    - 角色：关于权限位调整至普通用户角色；
    - 模型：向量模型支持模型参数设置；
    - 知识库：根目录显示当前工作空间下所有知识库，支持全局搜索。
