...
!!! Abstract ""
    ...
    压缩包内部必须包含两个文件：国际语言代码.json、国际语言代码.po
...
!!! Abstract ""
    ...
    MaxKB 默认提供中文语言包，语言包：zh.zip， 可基于中文语言包快速制作任意国际语言包，具体步骤如下：
...
!!! Abstract ""
    ...
    自定义语言包制作完成后，需放置到 MaxKB 指定的语言包目录，分为原生部署和Docker 容器部署两种场景。

### 3.1 原生部署（服务器直接安装）
!!! Abstract ""

    进入 MaxKB 安装目录的语言包文件夹，默认路径：/opt/maxkb/local/locales
    若该目录不存在，可手动创建目录：mkdir -p /opt/maxkb/local/locales
    将制作好的 xx.zip 自定义语言包，放入该目录下

### 3.2 Docker 镜像部署
!!! Abstract ""

    Docker 部署需通过目录映射，将宿主机语言包同步至容器内部指定目录，操作如下：
    在宿主机创建语言包目录（可自定义路径），放入自定义语言包 ZIP 文件
    启动/重启容器时，添加目录挂载参数，将宿主机目录映射至容器内： 容器固定语言包路径：/opt/maxkb/local/locales
    若容器内无 /opt/maxkb/local/locales 目录，可进入容器手动创建
