...
!!! Abstract ""
    对话用户是经过身份验证后，能够在问答页面进行提问的用户。管理员可以通过用户组管理功能，灵活控制哪些用户可以访问问答页面，并为不同用户组分配不同的权限。   
    权限说明：
...
## 3 登录认证
!!! Abstract ""
    系统管理员可以配置登录认证方式，对接第三方用户系统后，用户登录系统时将被分配到默认用户组。登录认证配置完成后，可以在【应用】- 【概览】 - 【访问限制】中显示该登录方式。
