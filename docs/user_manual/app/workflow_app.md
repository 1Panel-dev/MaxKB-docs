...
| ---- |-------------------------------------------------|----------------------------------------|
| **基础组件** |                                                 |                                        |
|  **AI 能力** | AI 对话、意图识别、图片生成、图片理解、文本转语音、语音转文本、文生视频、图生视频、问题优化 | 提供大模型调用及多模态能力                          |
|  **知识库** | 知识库检索、多路召回                                      | 对知识库内容进行语义/关键词检索                       |
|  **业务逻辑** | 判断器、表单收集、变量赋值、指定回复、循环节点                         | 控制流程分支、收集信息、修改变量、直接返回固定内容              |
|  **其他**  | 用户输入、接口传参、MCP 调用、文档内容提取、自定义工具                   | 获取外部输入、调用 MCP 工具、解析文件                  |
| **工具** | 已创建并启用的工具                                       | 将任意工具节点化，支持通过函数、脚本、API 等方式灵活处理复杂需求     |
...
!!! Abstract ""
    ...
    * AI 对话：与 AI 大模型进行对话节点。 
    * 意图识别：将用户问题与用户预设的意图分类进行匹配。
    * 图片生成：根据提供的文本内容生成图片。
    * 图片理解：识别并理解图片所包含的信息。
    * 文本转语音：将文本转换为语音。
    * 语音转文本：将音频转换为文本。
    * 文生视频：根据提供的文本内容生成视频。
    * 图生视频：根据提供的图片生成视频
    * 问题优化：AI 对话的一种，设定了默认的角色和提示词，根据上下文优化问题。   
    ...
    * 变量赋值：更新全局变量的值。
    * 指定回复：直接指定回复内容。
    * 循环节点：通过设置循环次数和逻辑，重复执行一系列任务。
    * MCP 调用：通过 SSE 协议调用 MCP 服务中的工具。
    * 文档内容提取：提取文档中的内容。
    * 自定义工具：通过执行自定义脚本，实现数据处理。
