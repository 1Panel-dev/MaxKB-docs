...
!!! Abstract ""
    执行记录：每次导入文档的操作会记录在工作流的【执行记录】中，用户可按发起人或状态，搜索指定执行记录，实现对所有导入操作的全链路追溯。
![执行记录工作流知识库](../../img/dataset/record_workflow_dataset.png)
![执行记录工作流知识库](../../img/dataset/record_workflow_dataset1.png)

## 2 知识库操作

!!! Abstract ""
    MaxKB 知识库支持同步、重新向量化、生成问题、资源授权、转移到、设置、导出以及删除操作。
