...
!!! Abstract ""
    工具的创建支持创建工具、创建 MCP、创建数据源、导入创建和工具商店。
...
!!! Abstract ""
    已启用的工具，可以在【高级编排应用】中，点击【添加组件】->【工具】中，以添加组件的方式调用工具；也可通过【AI 对话】组件的【工具】进行调用。
...
!!! Abstract ""
    工具添加后，在配置启动参数（例如数据库连接信息、API Key 等）并启用后，便可在高级编排应用中调用。
